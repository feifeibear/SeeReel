...
# reelyai-agent — Agent 操作手册

> **读者**：Codex、Claude Code、Cursor Agent 等 coding agent。  
> **不是**给终端用户的产品说明书。人只负责在初始化阶段按你的指引开通火山引擎服务、填写 `.env`、在 Web 审片；**整条制作链路由你通过 REST API 执行**。
...
人不开 API；人只：**按 Phase A 在火山引擎/console 开通服务并填 `.env`**，在 Web **审阅/改** 剧本、分镜、资产，必要时手动点生成/拼接。你把 `BASE_URL` 和当前 session 标题发给人即可。
