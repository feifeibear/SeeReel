...
- [ ] Stitch nodes do not create default `stitch -> audio` edges. Connecting a stitch node to an audio-track node persists that edge; deleting the edge removes only the optional audio-track wiring.
- [ ] Right-clicking the canvas can create a visible storyboard node, and visual reference edges among character, scene, and storyboard nodes persist to generation state and can be disconnected.
- [ ] `npm run smoke:image-upload-placeholder` passes and proves local character/scene image upload creates a session-scoped pending image asset with a local preview, pending tag, and upload label before the cloud asset is available.
