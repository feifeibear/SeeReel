...
- Like LibTV, SeeReel exposes audio as a visible canvas node. The right-click menu can create an audio-track node with shortcut `A`; it binds to the current Full video output and opens the post-production Inspector for narration or music. Creating this node must not restore a visible Stitch node.
- Final-video stitching is controlled from the toolbar's Full video panel rather than a dedicated canvas Stitch node.
- The canvas must not require a dedicated Stitch node. Like LibTV, users define the stitch sequence by connecting one video Shot node to the next video Shot node; the Full video panel reads that video chain, can persist it to `stitchShotIds`, and then runs the existing session-level stitch job.
...
- VLM review is feedback-only for prompt text. Review failures may be shown in the UI and audit state, but automatic prompt repair routes, prompt rewriters, and repair buttons must not write changes back into image, storyboard, shot, or final-video prompts.
...
- [ ] The canvas does not render or create Stitch nodes. Shot-to-shot video chain edges remain visible and the Full video panel can stitch by that connected order or fall back to shot order.
...
- [ ] Right-clicking the LibTV-style canvas can create a visible audio-track node with shortcut `A`; the node binds to the current Full video output, remains optional, and does not require or render a Stitch node.
...
- [ ] Run `npm run smoke:libtv-style-stitch` when canvas stitch UI, Shot-to-Shot video chain behavior, or right-click node creation changes.
