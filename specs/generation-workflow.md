...
- Image model-version selection belongs in the Image node Inspector, not on the canvas node. Video model-version selection belongs in the Video/Shot node Inspector, not in hidden prompt composition or canvas prompt tooling.
...
- Canvas previews for generated images and videos must remain valid independently of expiring Seedream/Seedance/TOS presigned URLs. Generated still images should persist a local `/media/...` preview for canvas display while retaining any remote `http(s)` URL for remote workers. Generated shot/final video canvas playback should prefer the local cached `/media/...` artifact before any remote playback/download URL.
...
- [ ] Image and video model-version controls live in their respective Inspectors; Image canvas nodes do not render model pickers.
