...
- Agent script-director workflows for narrative, satire, comedy, and dialogue-driven shorts, including story spine, scene objectives, character functions, cinematic shot language, and dialogue packets before render.
...
- Research-backed script development workflows that collect character, plot, and historical background material before first draft, then revise through review passes before final canvas handoff.
...
- After the first script draft, the agent must run script review iterations and patch the `StoryPlan`, characters, scenes, shot scripts, prompts, and `assetIds` until the reviewer is satisfied or a bounded review loop reaches unresolved blockers. New script ideas require at least two review passes before final canvas handoff.
...
- [ ] `npm run smoke:seereel-director-skill` also proves the director and script-chat skills prefer full 15-second Seedance shots, pack multiple related beats into one shot, and avoid one short clip per beat unless a cut is necessary for story continuity.
- [ ] `npm run smoke:seereel-director-skill` also proves the director skill treats visible shot seams as a continuity problem, prefers serial generation plus previous-tail clips for adjacent story continuity, and reserves parallel generation for truly independent visual branches.
...
- [ ] `npm run smoke:seereel-director-skill` also proves director and script-chat skills require `audienceDelivery`: important prompt information must be delivered through voiceover or character dialogue with visible action/reaction backup, not hidden in prompt-only lore, subtitles, readable signs, UI text, captions, or private notes.
...
- [ ] `npm run smoke:seereel-director-skill` also proves the director skill requires cinematic camera grammar: motivated camera movement or intentional lock-off, coverage ladder, 180-degree axis/screen direction, blocking before movement, attention target, and cut bridges such as match on action, eyeline match, reaction shot, insert, cutaway, sound bridge, previous-tail, or tailframe.
- [ ] `npm run smoke:seereel-director-skill` also proves the director, script-chat, and storyboard skills teach the Seedance shot-contract prompt structure, reference-role assignment, explicit shot size/framing, chronological motion beats, first-frame/reference-video mode hygiene, short trimmed reference-video usage, one-variable retry discipline, storyboard-grid avoidance, and final composed prompt dry-run before paid render.
...
- [ ] `npm run smoke:seereel-director-skill` also proves script-chat requires a research pass for characters, plot, and historical background, a script review loop after the first draft, multiple iterations until the reviewer is satisfied, and autonomous operation unless Interactive Mode is requested.
...
- [ ] `npm run smoke:seereel-director-skill` also proves the director skill teaches accepted rendered-frame identity anchors and rejects whole-cast reference pollution by requiring shot references to be visible or explicitly mentioned in that shot.
...
- [ ] `npm run smoke:seereel-canvas-review-skill` passes and proves the canvas-review skill covers research-backed review-first script expansion, character assets, scene assets, storyboards, 15-second beat-packed shot prompts, `assetIds` reference links, human approval, manual-edit refresh, script review iterations, autonomous non-interactive intake, and no video generation before approval.
...
- [ ] Run `npm run smoke:seereel-director-skill` when agent director, script-director, one-shot, first-frame, tailframe, route-planning, beat-ladder, dialogue-prompt, research-pass, script-review-loop, interactive-mode, style-bible, asset-style-lock, character-asset coverage, or consistency-gate skill behavior changes.
- [ ] Run `npm run smoke:seereel-director-skill` and `npm run smoke:seereel-canvas-review-skill` when orientationPacket, identityDelivery, opening exposition, first-appearance, or cold-viewer audit guidance changes.
...
- [ ] Run `npm run smoke:seereel-canvas-review-skill` when review-first canvas intake, research behavior, script review loop, interactive-mode behavior, script expansion, character/scene/storyboard planning, or shot-reference skill behavior changes.
