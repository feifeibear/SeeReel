...
| Volcengine Music access key ID | `VOLC_MUSIC_ACCESS_KEY_ID` | `VOLCENGINE_ACCESS_KEY`, `VOLCENGINE_ACCESS_KEY_ID`, `VOLC_ACCESS_KEY`, `VOLC_ACCESS_KEY_ID`, `TOS_ACCESS_KEY_ID`, `TOS_ACCESS_KEY` | Yes | AK for `doubao-music` OpenAPI signing on `open.volcengineapi.com`, `Service=imagination`; Agent Plan keys are not accepted for music. |
| Volcengine Music secret key | `VOLC_MUSIC_SECRET_ACCESS_KEY` | `VOLCENGINE_SECRET_KEY`, `VOLCENGINE_SECRET_ACCESS_KEY`, `VOLC_SECRET_KEY`, `VOLC_SECRET_ACCESS_KEY`, `TOS_SECRET_ACCESS_KEY`, `TOS_ACCESS_KEY_SECRET` | Yes | SK paired with the music AK. Must stay server-side and must never be shown in browser state. |
| Volcengine Music session token | `VOLC_MUSIC_SESSION_TOKEN` | `VOLCENGINE_SESSION_TOKEN`, `VOLCENGINE_STS_TOKEN` | Yes | Optional temporary credential token for music OpenAPI signing. Sent as `X-Security-Token` and included in signed headers. |
